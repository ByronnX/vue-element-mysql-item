      ...
      <el-form ref="LoginFormRef" :model="loginForm" :rules="LoginFormRules">
        ...
        <div style="margin: 10px 0; text-align: center">
          ...
          <el-button type="warning" size="small" @click="resetLoginForm"
            >重置</el-button
          >
          <el-button type="success" size="small" @click="toRegister"
            >注册</el-button
          >
        </div>
      </el-form>
...
export default {
  ...
  methods: {
    // login() {
    //   this.$axios
    //     .post("http://localhost/user/login", this.loginForm)
    //     .then((res) => {
    //       if (res.data) {
    //         this.$message.success("登录成功");
    //         this.$router.push("/dashboard");
    //       } else {
    //         this.$message.error("登陆失败!账号密码有误！");
    //       }
    //     })
    //     .catch((err) => {
      ...
      vm.$axios
        ...
        .then(function (response) {
          ...
          if (response.data.code == 200) {
            ...
            vm.$store.commit("setToken", response.data.data.token);
            vm.$store.commit("setUser", response.data.data.user);
            //跳转到登录成功后的页面
